...
func (r *NvmetHostSubsysResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if err != nil {
		resp.Diagnostics.AddError("Read Error", fmt.Sprintf("Created but failed to read back nvmet_host_subsys: %s", err))
		return
	}
...
func (r *NvmetHostSubsysResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Read back to populate computed fields
	result, readErr := r.client.Call("nvmet.host_subsys.get_instance", id)
	if readErr != nil {
		resp.Diagnostics.AddError("Read Error", fmt.Sprintf("Updated but failed to read back nvmet_host_subsys: %s", readErr))
		return
	}
	resultMap, ok := result.(map[string]interface{})
	if !ok {
