...
func (r *JbofResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if data.MgmtIp2.IsUnknown() {
		data.MgmtIp2 = types.StringNull()
	}
...
func (r *JbofResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Read back to populate computed fields
	result, readErr := r.client.Call("jbof.get_instance", id)
	if readErr != nil {
		resp.Diagnostics.AddError("Read Error", fmt.Sprintf("Updated but failed to read back jbof: %s", readErr))
		return
	}
	resultMap, ok := result.(map[string]interface{})
	if !ok {
