...
func (r *StaticrouteResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Read back to populate computed fields
	result, readErr := r.client.Call("staticroute.get_instance", id)
	if readErr != nil {
		resp.Diagnostics.AddError("Read Error", fmt.Sprintf("Updated but failed to read back staticroute: %s", readErr))
		return
	}
	resultMap, ok := result.(map[string]interface{})
	if !ok {
