...
func (d *VirtInstancesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, item := range resultList {
		...
		if v, ok := resultMap["vnc_enabled"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.VncEnabled = types.BoolValue(bv) }
		}
		if v, ok := resultMap["vnc_port"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.VncPort = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["secure_boot"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.SecureBoot = types.BoolValue(bv) }
		}
		if v, ok := resultMap["privileged_mode"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.PrivilegedMode = types.BoolValue(bv) }
		}
		if v, ok := resultMap["root_disk_size"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.RootDiskSize = types.Int64Value(int64(fv)) }
		}
