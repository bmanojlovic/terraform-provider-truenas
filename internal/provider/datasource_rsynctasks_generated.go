...
func (d *RsynctasksDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, item := range resultList {
		...
		if v, ok := resultMap["remoteport"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.Remoteport = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["times"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Times = types.BoolValue(bv) }
		}
		if v, ok := resultMap["compress"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Compress = types.BoolValue(bv) }
		}
		if v, ok := resultMap["archive"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Archive = types.BoolValue(bv) }
		}
		if v, ok := resultMap["delete"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Delete = types.BoolValue(bv) }
		}
		if v, ok := resultMap["quiet"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Quiet = types.BoolValue(bv) }
		}
		if v, ok := resultMap["preserveperm"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Preserveperm = types.BoolValue(bv) }
		}
		if v, ok := resultMap["preserveattr"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Preserveattr = types.BoolValue(bv) }
		}
		if v, ok := resultMap["delayupdates"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Delayupdates = types.BoolValue(bv) }
		}
