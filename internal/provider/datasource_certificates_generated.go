...
func (d *CertificatesDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	for _, item := range resultList {
		...
		if v, ok := resultMap["type"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.Type = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["renew_days"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.RenewDays = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["add_to_trusted_store"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.AddToTrustedStore = types.BoolValue(bv) }
		}
		...
		if v, ok := resultMap["cert_type_existing"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.CertTypeExisting = types.BoolValue(bv) }
		}
		if v, ok := resultMap["cert_type_CSR"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.CertTypeCsr = types.BoolValue(bv) }
		}
		if v, ok := resultMap["cert_type_CA"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.CertTypeCa = types.BoolValue(bv) }
		}
		if v, ok := resultMap["key_length"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.KeyLength = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["subject_name_hash"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.SubjectNameHash = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["lifetime"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.Lifetime = types.Int64Value(int64(fv)) }
		}
		...
		if v, ok := resultMap["serial"]; ok && v != nil {
			if fv, ok := v.(float64); ok { itemModel.Serial = types.Int64Value(int64(fv)) }
		}
		if v, ok := resultMap["chain"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Chain = types.BoolValue(bv) }
		}
		...
		if v, ok := resultMap["expired"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Expired = types.BoolValue(bv) }
		}
		if v, ok := resultMap["parsed"]; ok && v != nil {
			if bv, ok := v.(bool); ok { itemModel.Parsed = types.BoolValue(bv) }
		}
